...
>
> 产品定位：**本地优先的工作管理平台**。核心能力是对接**钉钉**与**邮箱（Outlook）**数据，并借助 **LLM 进行数据分析**；**所有数据（含团队日报、组织关系、审计）全部存放到本地 SQLite**，不再使用独立团队服务与 MySQL，多用户按身份隔离。
>
> 因此，所有"知识库相关模块"（Overview 总览、知识图谱、Wiki、素材库、书架、主题、文档阅读器和全文搜索）以及**社媒洞察、抖音数据**等 Vault 内容展示模块的**导航入口均已隐藏，不对用户开放**（代码与路由仍保留，不作为对外功能）。每日热点（AI HOT 匿名 API）仍保留。具体隐藏清单见第二节。

---

## 一、项目一句话定位

**Personal AI Workbench 是一个"本地优先、以钉钉 + 邮箱数据为工作对象"的个人工作管理平台。** 应用负责把钉钉日程/待办/日报、Outlook 邮件等数据汇总到统一的今日行动与日报闭环中，并借助 LLM 提供邮件分类、日报草稿、周报摘要等数据分析能力；数据（含团队日报、组织关系、审计）全部存放在本地 SQLite，多用户按身份隔离。默认只在本机运行，外部服务（钉钉、Outlook、DeepSeek）需配置后启用。

> 注：仓库代码中仍保留 Markdown Vault 的索引与展示能力，但作为产品方向，**知识库、社媒洞察与抖音数据模块均暂不开发、不对用户开放**（隐藏清单见下节）。
...
页面路由集中在 `src/App.jsx`。按产品能力归为 6 大域，其中"知识库相关模块"与"社媒/抖音内容展示"当前**全部隐藏、不对用户开放**（含代码层面保留、不作为对外功能）。

### 0. 已隐藏模块：知识库 + 社媒/抖音（暂不开发，不对用户开放）
以下页面/能力的导航入口已隐藏，不对用户开放，也不投入产品迭代（代码与路由仍保留，不作为对外功能）：
...
| 社媒洞察 | `/social-insights` | Vault 中 `10_raw/social-insights/**` 内容展示 |
| 抖音数据 | 组件内嵌（当前未挂接到任何可见路由） | Vault 中 `30_self_media/douyin/current.json` 内容展示 |
...
系统的存储职责边界清晰，**同一事实不应在两处重复维护**。产品定位下，**所有数据（含团队日报、组织关系、审计）完全存放到本地 SQLite**：

```text
【工作数据源】钉钉 + Outlook + 本地操作  ← 外部平台数据经同步入库，LLM 仅作分析不存原文
        │  同步/索引 server/ 各领域服务（含团队日报与组织关系，不再依赖独立服务或 MySQL）
        ▼
【本地数据库】SQLite (.local/workbench.sqlite)  ← 任务、周重点、日报、团队日报、组织、审计等全部数据
...
【保留数据】Vault (Markdown/CSV/JSON) ← 仅演示用途；知识库/社媒/抖音模块不开放
```
...
5. **不重新引入独立的团队服务与 MySQL**：独立的 `8787` 团队服务和 MySQL 已移除，团队日报、组织关系与审计数据全部存本地 SQLite；当前页面数据必须走 `server/vite-plugin-workbench.mjs` 的本地接口，不要用演示数据作为认证失败或同步失败的回退。
...
8. **知识库与社媒/抖音模块不开放**：Overview、图谱、Wiki、素材库、书架、主题、文档阅读器、全文搜索，以及社媒洞察、抖音数据均不对用户开放（隐藏清单见第二节第 0 项）。每日热点仍保留；如产品上需要调整隐藏/开放范围，必须先在本指南与相关文档中同步说明。
...
- 在"知识库、社媒/抖音不开放"的方向下，Vault 数据契约当前**已无对外功能用途**（对应模块均已隐藏），通常无需调整；如后续产品上恢复开放，契约改动需同步 4 处：
...
- 侧边导航入口在 `src/components/AppShell.jsx` 的导航配置数组中；知识库、社媒/抖音页面均不在该配置中（即导航入口已隐藏，路由代码仍保留）。

### 场景 G：调整"知识库 / 社媒 / 抖音模块"的隐藏、开放范围
...
2. **知识库与社媒/抖音不开放是产品红线**：Overview/图谱/Wiki/素材/书架/主题/阅读器/搜索，以及社媒洞察/抖音数据均不对用户开放；调整隐藏范围必须同步文档（见红线第 8 条）。
3. **Vault 契约已无对外用途**：知识库、社媒/抖音模块均隐藏；若日后恢复开放，契约改动仍需四处同步（索引、fallback、文档、测试）。
4. **所有数据统一存本地 SQLite**：团队日报、组织关系、审计与个人状态同库存储，不再有独立团队服务/MySQL；注意"内容/演示数据"与"本地应用状态"的职责边界，避免同一事实在多处写入。
