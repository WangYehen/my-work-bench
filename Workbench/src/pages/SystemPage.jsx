...
export function SystemPage() {
  ...
  const syncLabel = ({ watching: "正在监听文件变化", idle: "等待文件变化", syncing: "正在同步", error: "同步异常" })[sync?.status] || (sync?.status ? "状态已更新" : "尚未启动");
  ...
  return <div className="page page--system">
    ...
    <div className="system-grid"><div className="panel"><div className="panel__head"><h2 className="panel__title">Vault 索引</h2></div><div><div className="system-kv"><dt>标签</dt><dd>{vault?.label || "本地 Vault"}</dd></div><div className="system-kv"><dt>文档数</dt><dd>{vault?.documents ?? "—"}</dd></div><div className="system-kv"><dt>索引时间</dt><dd>{formatFullDate(vault?.generatedAt)}</dd></div><div className="system-kv"><dt>错误数</dt><dd>{vault?.errors ?? "—"}</dd></div><div className="system-kv"><dt>文件同步</dt><dd>{syncLabel}</dd></div></div></div><div className="panel"><div className="panel__head"><h2 className="panel__title">Codex 运行时</h2></div><div><div className="system-kv"><dt>可用性</dt><dd>{codex?.available ? "可用" : "不可用"}</dd></div><div className="system-kv"><dt>来源</dt><dd>{codex?.source === "path" ? "本机安装" : codex?.source || "尚未检测"}</dd></div></div></div></div>
  </div>;
}
