...
const emptyDashboard = () => ({ reportDate: today(), generatedAt: new Date().toISOString(), metrics: { submitted: 0, expected: 0, missing: 0, late: 0 }, members: [], reports: [], departments: [], summary: { blockers: [], decisions: [], nextActions: [], mode: "" } });
// 日报明细中的字段可能是字符串或数组，统一转成可展示的文本
...
export function TeamReportsAdminPage() {
  ...
  return <div className="page team-dashboard-page">
    ...
    <div className="team-dashboard-grid">
      <section className="team-dashboard-panel"><h2>团队共同阻塞</h2><div className="team-blocker-list">{dashboard.summary.blockers.length ? dashboard.summary.blockers.map((item, index) => <article key={`${item.text}-${index}`}><i /><div><strong>{item.text}</strong><small>涉及：{item.departments?.join("、") || "跨团队"}</small></div><span>{item.affectedMembers?.length || 1} 人</span><b>{severityLabel[item.severity] || "中"}</b></article>) : <div className="report-empty"><strong>今日暂无团队阻塞</strong><span>成员日报同步后，团队共同阻塞会聚合到这里。</span></div>}</div><button className="team-panel-link" type="button">查看全部阻塞（{dashboard.summary.blockers.length}）<IconChevronRight size={14} /></button></section>
      <section className="team-dashboard-panel"><h2>需要领导决策</h2><div className="team-decision-list">{dashboard.summary.decisions.length ? dashboard.summary.decisions.map((item, index) => <article className={decided.has(index) ? "is-decided" : ""} key={`${item.text}-${index}`}><div><strong>{item.text}</strong><small>{item.owner || "待指定负责人"}</small>{item.priority ? <b className={`is-p${item.priority.slice(1)}`}>{item.priority}</b> : null}</div><time>{item.dueAt || "尽快"}</time><button onClick={() => toggleDecided(index)} type="button">{decided.has(index) ? "已决策" : "标记已决策"}</button></article>) : <div className="report-empty"><strong>今日暂无待决策事项</strong><span>需要领导介入的事项会聚合到这里。</span></div>}</div><button className="team-panel-link" type="button">查看全部决策<IconChevronRight size={14} /></button></section>
      <section className="team-dashboard-panel"><h2>明日关键行动</h2><div className="team-action-list">{dashboard.summary.nextActions.length ? dashboard.summary.nextActions.map((item, index) => <article key={`${item.text}-${index}`}><i /><strong>{item.text}</strong><span>{item.owner || "待认领"}</span><small>{item.departmentName || "跨团队协作"}</small></article>) : <div className="report-empty"><strong>今日暂无明日行动</strong><span>成员日报同步后，明日工作计划会聚合到这里。</span></div>}</div></section>
    ...
    {detailsOpen ? <section className="team-report-list">{dashboard.reports.length ? dashboard.reports.map((report) => <article className="team-report-card" key={report.id}><div className="team-report-card__head"><strong>{report.displayName}</strong><time>{report.submittedAt}</time></div><h3>{report.summary}</h3><dl><div><dt>阻塞</dt><dd>{asText(report.blockers) || "无"}</dd></div><div><dt>协作</dt><dd>{asText(report.cooperationNeeds) || "无"}</dd></div><div><dt>下一步</dt><dd>{asText(report.nextActions) || "无"}</dd></div></dl>{(() => { let count = 0; try { count = JSON.parse(report.attachments || "[]").length; } catch { count = 0; } return count ? <small className="team-report-card__attachments">含 {count} 个附件</small> : null; })()}</article>) : <div className="report-empty"><strong>当日暂无日报</strong><span>点击"同步日志"拉取团队成员当日日报后，这里展示你权限范围内的日报文本。</span></div>}</section> : null}
  </div>;
}
